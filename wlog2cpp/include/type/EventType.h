...
namespace types
{
  ...
  inline const char* getCName(EventType type)
  {
    return EVENT_TYPE_NAMES[(size_t) type];
  }

  inline string_t getName(EventType type)
  {
    return std::string(getCName(type));
  }

  inline EventType getEventType(string_t name)
  {
    return (EventType) findNameInArray(name, EVENT_TYPE_NAMES, EVENT_TYPE_NAMES_COUNT);
  }
